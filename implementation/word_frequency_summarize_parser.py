...
# Important: These scores are for the experimenting purpose
WEIGHT_FOR_LIST = 5
...
def _create_frequency_table(paragraph_list) -> dict:
    ...
    for paragraph in paragraph_list:
        ...
        all_highlighted_sentences_list = [sent for sent in paragraph.get_highlighted()]
        highlighted_words_text1 = " ".join(all_highlighted_sentences_list)
        highlighted_words1 = word_tokenize(highlighted_words_text1)
...
def _score_sentences(sentences, freqTable) -> dict:
    ...
    for each_sentence in sentences:
        word_count_in_sentence = (len(word_tokenize(each_sentence)))
        word_count_in_sentence_except_stop_words = 0
        for wordValue in freqTable:
            if wordValue in each_sentence.lower():
                word_count_in_sentence_except_stop_words += 1
                if each_sentence[:10] in sentence_Value:
                    sentence_Value[each_sentence[:10]] += freqTable[wordValue]
                else:
                    sentence_Value[each_sentence[:10]] = freqTable[wordValue]

        if each_sentence[:10] in sentence_Value:
            sentence_Value[each_sentence[:10]] = sentence_Value[each_sentence[:10]] / word_count_in_sentence_except_stop_words
...
def _generate_summary(sentences, sentenceValue, thr):
    sentence_count = 0
    summary_text = ''

    for each_sentence in sentences:
        if each_sentence[:10] in sentenceValue and sentenceValue[each_sentence[:10]] >= (thr):
            summary_text += " " + each_sentence
            sentence_count += 1
...
def run_summarization(paragraph_list):
    ...
    frequency_table = _create_frequency_table(paragraph_list)
    

    # 2 Tokenize the sentences
    sentences_list = [paragraph.text for paragraph in paragraph_list]
    

    # 3 Important Algorithm: score the sentences
    sentence_score = _score_sentences(sentences_list, frequency_table)

    # 4 Find the threshold
    thr = _find_average_score(sentence_score)

    # 5 Important Algorithm: Generate the summary
    summary_text = _generate_summary(sentences_list, sentence_score, 1.3 * thr)
